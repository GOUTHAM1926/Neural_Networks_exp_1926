// test_bgemm_sm103_bf16_nt_cluster.cu
// Benchmark and accuracy test: BluBridge SM103a BF16 batched NT Cluster GEMM vs
// cuBLAS. Target: NVIDIA B300 (SM 10.3a) — tcgen05.mma.cta_group::2.kind::f16
//
// Usage:
//   ./build/test_bgemm_sm103_bf16_nt_cluster
//
...
static Result run_test(int M, int N, int K, int batchCount, float alpha,
  ...
  // ── cuBLAS reference (batched NT sgemm) ────────
  {
    float fa = alpha, fb = beta;
    CHECK_CUBLAS(cublasGemmStridedBatchedEx(
        cublas, CUBLAS_OP_T, CUBLAS_OP_N, N, M, K, &fa, dB, CUDA_R_16BF, K,
        (long long)N * K, dA, CUDA_R_16BF, K, (long long)M * K, &fb, dC_ref,
...
// cuBLAS picks. Runs 20 timed iters of the NT cuBLAS call.
static double run_cublas_only_nt(int M, int N, int K, int batchCount,
  ...
  for (int w = 0; w < 5; ++w) {
    CHECK_CUBLAS(cublasGemmStridedBatchedEx(
        cublas, CUBLAS_OP_T, CUBLAS_OP_N, N, M, K, &fa, dB, CUDA_R_16BF, K,
        (long long)N * K, dA, CUDA_R_16BF, K, (long long)M * K, &fb, dC,
        CUDA_R_16BF, N, (long long)M * N, batchCount, CUBLAS_COMPUTE_32F,
        CUBLAS_GEMM_DEFAULT_TENSOR_OP));
  }
  cudaEvent_t t0, t1;
...
int main() {
  ...
  if (prop.major != 10 || prop.minor != 3) {
    printf(
        "[SKIP] SM103 BF16 NT Cluster kernel requires SM 10.3 (B300/sm_103a).\n"
  ...
  printf("\n=== Testing 128x128x64 NT Cluster Kernel (cta_group::2) ===\n");
  ...
  printf("128x128x64 NT Results: %d / %d PASSED\n", passed, total);

  printf("\n=== Testing 256x256x64 NT Cluster Kernel (cta_group::2) ===\n");
  printf("%-20s %6s %6s %6s %5s  %-8s  %-8s  %-10s  %s\n", "Shape", "M", "N",
         "K", "Batch", "BluBLAS", "cuBLAS", "MaxRelErr", "Status");
  printf("%s\n", std::string(85, '-').c_str());
  ...
  printf("256x256x64 NT Results: %d / %d PASSED\n\n", passed_256, total_256);
  ...
  const std::vector<Shape> shapes_profile = {
      ...
      {8, 4096, 4096, 1, "decode-8"},   // small M: expect gemmSN_NT
      {32, 4096, 4096, 1, "decode-32"}, // small M: expect gemmSN_NT
  ...
  printf(
      "\n=== cuBLAS-only Profile Sweep NT (nsys capture — no BluBLAS) ===\n");
  printf("%-20s %6s %6s %6s %5s  %-8s\n", "Shape", "M", "N", "K", "Batch",
         "cuBLAS");
  printf("%s\n", std::string(65, '-').c_str());
  for (const auto &s : shapes_profile) {
    double tfl_p = run_cublas_only_nt(s.M, s.N, s.K, s.batch, cublas, stream);
  ...
  printf(
      "=== BluBLAS vs cuBLAS Qwen Sweep NT (M=4096, 8 models × ~5 ops) ===\n");
  ...
  printf("Qwen Sweep NT Results: %d / %d PASSED\n\n", qwen_pass, qwen_total);
