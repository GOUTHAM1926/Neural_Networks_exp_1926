...
// Benchmark and accuracy test: BluBridge SM103a BF16 batched TN Cluster GEMM vs
...
static Result run_test(int M, int N, int K, int batchCount, float alpha,
  ...
  // Since B is [K,N] row-major -> dB is [N,K] col-major. We use OP_N to get B^T
  // [N,K]. Since A is [K,M] row-major -> dA is [M,K] col-major. We use OP_T to
  // get A [K,M].
  {
    float fa = alpha, fb = beta;
    CHECK_CUBLAS(cublasGemmStridedBatchedEx(
        cublas, CUBLAS_OP_N, CUBLAS_OP_T, N, M, K, &fa, dB, CUDA_R_16BF, N,
        (long long)K * N, dA, CUDA_R_16BF, M, (long long)M * K, &fb, dC_ref,
...
// cuBLAS picks. Runs 20 timed iters of the TN cuBLAS call.
static double run_cublas_only_tn(int M, int N, int K, int batchCount,
  ...
  for (int w = 0; w < 5; ++w) {
    CHECK_CUBLAS(cublasGemmStridedBatchedEx(
        cublas, CUBLAS_OP_N, CUBLAS_OP_T, N, M, K, &fa, dB, CUDA_R_16BF, N,
        (long long)K * N, dA, CUDA_R_16BF, M, (long long)M * K, &fb, dC,
        CUDA_R_16BF, N, (long long)M * N, batchCount, CUBLAS_COMPUTE_32F,
        CUBLAS_GEMM_DEFAULT_TENSOR_OP));
  }
  cudaEvent_t t0, t1;
...
int main() {
  ...
  if (prop.major != 10 || prop.minor != 3) {
    printf(
        "[SKIP] SM103 BF16 TN Cluster kernel requires SM 10.3 (B300/sm_103a).\n"
  ...
  const std::vector<Shape> shapes_profile = {
      ...
      {8, 4096, 4096, 1, "decode-8"},   // small M: expect gemmSN_TN
      {32, 4096, 4096, 1, "decode-32"}, // small M: expect gemmSN_TN
  ...
  printf(
      "\n=== cuBLAS-only Profile Sweep TN (nsys capture — no BluBLAS) ===\n");
  printf("%-20s %6s %6s %6s %5s  %-8s\n", "Shape", "M", "N", "K", "Batch",
         "cuBLAS");
  printf("%s\n", std::string(65, '-').c_str());
  for (const auto &s : shapes_profile) {
    double tfl = run_cublas_only_tn(s.M, s.N, s.K, s.batch, cublas, stream);
  ...
  // 5 unique ops per model: Q proj, K/V proj, [O if non-square], Gate/Up, Down,
  // LM head. Naming: q25/q3 = series, then size, then op.
  const std::vector<Shape> shapes_qwen = {
      ...
      // Qwen 2.5-1.5B  hidden=1536  KV_dim=256   I=8960   vocab=151936
      ...
      // Qwen 2.5-3B   hidden=2048  KV_dim=256   I=11008  vocab=151936
  ...
  printf(
      "=== BluBLAS vs cuBLAS Qwen Sweep TN (M=4096, 8 models × ~5 ops) ===\n");
  ...
  printf("Qwen Sweep TN Results: %d / %d PASSED\n\n", qwen_pass, qwen_total);
