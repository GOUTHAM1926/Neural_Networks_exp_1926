...
//
// Each cluster:  CTA0 → A rows [block_row0*BM..(block_row0+1)*BM), B cols
// [block_col*BN .. +BN/2)
//                CTA1 → A rows [block_row1*BM..(block_row1+1)*BM), B cols
...
static inline void init_4d_tma_B_NT(CUtensorMap *tmap, const __nv_bfloat16 *ptr,
                                    int N, int K, int BN, int BK,
                                    uint64_t batchCount, uint64_t batchStride,
                                    CUtensorMapSwizzle sw) {
  constexpr uint32_t rank = 4;
  uint64_t globalDim[rank] = {64ULL, (uint64_t)N, (uint64_t)(K / 64),
                              batchCount};
  uint64_t globalStrides[rank - 1] = {(uint64_t)K * sizeof(__nv_bfloat16),
                                      128ULL,
                                      batchStride * sizeof(__nv_bfloat16)};
  uint32_t boxDim[rank] = {64U, (uint32_t)(BN / CTA_GROUP_SIZE),
                           (uint32_t)(BK / 64), 1U};
  uint32_t elementStrides[rank] = {1U, 1U, 1U, 1U};
  check_cu(cuTensorMapEncodeTiled(
      tmap, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, rank, (void *)ptr, globalDim,
...
__global__ __cluster_dims__(2, 1, 1) void bgemm_sm103_bf16_nt_cluster_kernel(
  ...
  if (warp_id == producer_warp && elect_sync()) {
    ...
    while (true) {
      ...
      for (int k = 0; k < k_iters; ++k) {
        ...
        tma_4d_gmem2smem<CTA_GROUP_SIZE>(B_smem, &B_tmap, 0, B_row, k,
                                         cur_batch, mbar);
...
static void
launch_bgemm_nt_cluster(const __nv_bfloat16 *A, const __nv_bfloat16 *B,
  ...
  if (s_arch_ok < 0) {
    ...
    if (!s_arch_ok)
      fprintf(stderr,
              "[BluBridge SM103 NT Cluster] WARNING: device '%s' SM %d.%d, "
              "requires SM 10.3+\n",
  ...
  init_4d_tma_B_NT(&B_tmap, B, N, K, BN, BK, (uint64_t)batchCount,
                   (uint64_t)N * K, CU_TENSOR_MAP_SWIZZLE_128B);
...
static void dispatch_bgemm_nt_cluster(const __nv_bfloat16 *A,
                                      const __nv_bfloat16 *B, __nv_bfloat16 *C,
                                      int M, int N, int K, int batchCount,
                                      float alpha, float beta,
                                      cudaStream_t stream) {
  launch_bgemm_nt_cluster<128, 128, 64>(A, B, C, M, N, K, batchCount, alpha,
                                        beta, stream);
}
