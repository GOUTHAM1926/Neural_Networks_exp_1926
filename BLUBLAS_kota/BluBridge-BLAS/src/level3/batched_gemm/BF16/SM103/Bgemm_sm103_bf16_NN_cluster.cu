...
// (block_row1 = block_row0 + 1 via Hilbert map), columns [block_col*BN ..
// (block_col+1)*BN-1]. CTA0 loads the low  BN/2 columns of B; CTA1 loads the
// high BN/2 columns.
...
static constexpr int SWIZZLE_W =
    64; // 64 elements = 128 bytes per swizzle group
...
__device__ __forceinline__ uint64_t make_smem_desc(int addr) {
  ...
  return desc_encode(addr) | (desc_encode(stride) << 32ULL) | (1ULL << 46ULL) |
         (2ULL << 61ULL);
}
...
static inline void init_4d_tma_A(CUtensorMap *tmap, const __nv_bfloat16 *ptr,
                                 int M, int K, int BM, int BK,
                                 uint64_t batchCount, uint64_t batchStride,
                                 CUtensorMapSwizzle swizzle) {
  constexpr uint32_t rank = 4;
  uint64_t globalDim[rank] = {64ULL, (uint64_t)M, (uint64_t)(K / 64),
                              batchCount};
  uint64_t globalStrides[rank - 1] = {(uint64_t)K * sizeof(__nv_bfloat16),
                                      128ULL,
  ...
  check_cu(cuTensorMapEncodeTiled(tmap, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, rank,
                                  (void *)ptr, globalDim, globalStrides, boxDim,
                                  elementStrides, CU_TENSOR_MAP_INTERLEAVE_NONE,
                                  swizzle, CU_TENSOR_MAP_L2_PROMOTION_NONE,
                                  CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE));
}

// 4D TMA for B NN: B[batch, K, N] row-major, box = [64, BK,
// BN/(CTA_GROUP_SIZE*64), 1]
static inline void init_4d_tma_B_NN(CUtensorMap *tmap, const __nv_bfloat16 *ptr,
...
__global__ __cluster_dims__(2, 1, 1) void bgemm_sm103_bf16_nn_cluster_kernel(
    const __nv_bfloat16 *__restrict__ A, // [batch, M, K] row-major
    const __nv_bfloat16
        *__restrict__ C_in,        // [batch, M, N] row-major (for beta)
    __nv_bfloat16 *__restrict__ C, // [batch, M, N] row-major (output)
    int M, int N, int K,
    int total_mn_tiles, // (M/(CTA_GROUP_SIZE*BM)) * (N/BN)
    float alpha, float beta, const __grid_constant__ CUtensorMap A_tmap,
    ...
    int grid_n) // number of BN-wide column tiles (N/BN)
{
  ...
  // [QUEUE_SIZE × (BM*BK + (BN/2)*BK)] pipeline stages, then store
  // double-buffer
...
#pragma nv_diag_suppress static_var_with_dynamic_init
  ...
  constexpr int b_tile_bytes =
      (BN / CTA_GROUP_SIZE) * BK * (int)sizeof(__nv_bfloat16);
  ...
  if (warp_id == producer_warp && elect_sync()) {
    ...
    while (true) {
      ...
      for (int k = 0; k < k_iters; ++k) {
        ...
        tma_4d_gmem2smem<CTA_GROUP_SIZE>(A_smem, &A_tmap, 0, A_row, k,
                                         cur_batch, mbar);
          ...
          for (int k1 = 0; k1 < BK / SWIZZLE_W; ++k1) {
            for (int k2 = 0; k2 < SWIZZLE_W / MMA_K_STEP; ++k2) {
              ...
              tcgen05_mma_bf16<CTA_GROUP_SIZE>(
                  tmem_addr + wave_stage * BN, make_smem_desc(A_smem + a_off),
                  make_smem_desc(B_smem + b_off), i_desc, use_accum);
            }
          }
...
static void
launch_bgemm_nn_cluster(const __nv_bfloat16 *A, const __nv_bfloat16 *B,
  ...
  if (s_arch_ok < 0) {
    ...
    if (!s_arch_ok)
      fprintf(stderr,
              "[BluBridge SM103 NN Cluster] WARNING: device '%s' SM %d.%d, "
              "requires SM 10.3+\n",
  ...
  init_4d_tma_A(&A_tmap, A, M, K, BM, BK, (uint64_t)batchCount, (uint64_t)M * K,
                CU_TENSOR_MAP_SWIZZLE_128B);
  init_4d_tma_B_NN(&B_tmap, B, K, N, BK, BN, (uint64_t)batchCount,
  ...
  // Ceil-div so shapes smaller than one 2*BM=256 cluster tile still launch
  // (floor-div gives grid_m=0 → CUDA rejects launch as "invalid argument").
  ...
  kernel<<<grid, block_size, smem_size, stream>>>(
      A, C, C, M, N, K, total_mn_tiles, alpha, beta, A_tmap, B_tmap, C_tmap,
      grid_n);
...
static void dispatch_bgemm_nn_cluster(const __nv_bfloat16 *A,
  ...
  launch_bgemm_nn_cluster<128, 128, 64>(A, B, C, M, N, K, batchCount, alpha,
                                        beta, stream);
}
...
extern "C" void mycublasBgemmSM103_bf16_nn_cluster_256x256x64(
  ...
  // supports M ∈ {128, 256}. BM=256 (which needs M=512 per MMA) is
  // architecturally invalid on this instruction path — attempting to launch
  // it raises "illegal instruction" at runtime.
  // The 128x128x64 cluster kernel already produces a 256-row output tile per
  // cluster (2 CTAs × BM=128), so it IS the real production kernel for this
  // API. This entry is kept as a placeholder for a future higher-throughput
  // implementation (cta_group::1 with BM=256, OR BM=128 + BN=256).
  dispatch_bgemm_nn_cluster(A, B, C, M, N, K, batchCount, alpha, beta, stream);
}
