...
// void register_process_group(std::string group_name, std::shared_ptr<ProcessGroupNCCL> process_group){
// 	register_group(group_name, process_group);
// }


// //init commuication
// std::shared_ptr<ProcessGroupNCCL> init_process_group(int world_size, int rank, cudaStream_t stream, const std::string group_name){
...
//     /*
//      * Addition of the group registry
//      * Since the map and the global counter is not confined inside any class or function,
//      * It will not be created newly for every new instance
//      * 
//      * */
    
//     auto name = group_name.length() == 0 ? get_group_name() : group_name;
...
//     // Register the process group here.
//     register_process_group(name, pg);
...
//     // std::cout << "Completed there" << std::endl;
//     if(to_sync){
...
//all reduce (reuse-capable): recycles `reuse` instead of allocating a new Work.
std::shared_ptr<Work> ProcessGroupNCCL::all_reduce_async(const void* sendbuff, void* recvbuff,size_t count, OwnTensor::Dtype dtype, op_t operation, bool sync_, std::shared_ptr<Work> reuse){
    return launch_work_collectives(
        communication_stream_,
        [&]() -> ncclResult_t{
            return ncclAllReduce(sendbuff, recvbuff, count, ncclTypeConversion(dtype), ncclOperationConversion(operation), comm_, communication_stream_);
...
template<typename NCCLFUNC>
std::shared_ptr<Work> ProcessGroupNCCL::launch_work_collectives( cudaStream_t stream, NCCLFUNC nccl_op, bool to_sync, std::shared_ptr<Work> reuse ){
    //create the work collective — or recycle the caller's existing Work
    //(re-arming reuses its CUDA event instead of creating/destroying one).
    std::shared_ptr<Work> work_collectives;
    if(reuse){
        work_collectives = reuse;
        work_collectives->rearm(stream, comm_);
    }else{
        work_collectives = std::make_shared<Work>(stream, comm_);
        PG_CHECK_FALSE(work_collectives->begin(), "Error in work_collective->begin()");
    }
