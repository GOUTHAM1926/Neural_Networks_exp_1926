...
public:
    ...
    Embedding() = default;
    Embedding(int64_t vocab_size, int64_t embed_dim, DeviceIndex device, uint64_t seed = 1337)
...
void init_linear_gpt2(nn::Linear& layer, float std = 0.02f, uint64_t seed = 1337, bool req_grad=true) {
...
public:
    ...
    Attention(int64_t n_embd, int n_heads, int n_layers, DeviceIndex device, uint64_t seed = 1337)
        : ln(n_embd),
...
public:
    ...
    MLP(int64_t n_embd, int n_layers, DeviceIndex device, uint64_t seed = 1337)
        : ln(n_embd),
...
public:
    ...
    GPT(GPTConfig cfg, DeviceIndex device, uint64_t seed = 1337)
        : config(cfg), 
...
int main() {
    try {
        ...
        // Create CSV log file
        std::ofstream log_file("./train_log.csv");
        ...
        std::vector<Shape> shapes_to_log; // For debugging tensor shapes
        for (int step = start_step; step < MAX_STEPS; ++step) {
            ...
            //     std::vector<int64_t> xgen_tokens = {40, 716, 257, 6253,40, 716, 257, 6253,40, 716, 257, 6253,40, 716, 257, 6253};
            ...
            //         Tensor scaled_logits = last_logits_3d / temperature;
            ...
            // Training step
            //
            // Zero gradients. optimizer.zero_grad() routes through the
            // multi-tensor zero kernel (Optim.cpp) — all registered params
            // are zeroed in a handful of kernel launches rather than one
            // cudaMemsetAsync per param.
            optimizer.zero_grad();

            // When weight_tying is enabled, lm_head->weight is NOT registered
            // as a parameter (it's a transposed view of wte.weight). Autograd
            // still gives it its own leaf grad buffer, so it needs a manual
            // zero here or stale gradients accumulate across steps.
            if (model.config.weight_tying && model.lm_head->weight.has_grad()) {
                cudaMemsetAsync(model.lm_head->weight.grad(), 0,
                                model.lm_head->weight.nbytes());
            }
            ...
            for (auto& p : params) {
                if (p.has_grad()) {
                    shapes_to_log.push_back(p.grad_view().shape()); // Log shapes for debugging
                }
            }
            ...
            // Enqueue optimizer kernels BEFORE syncing on the loss — this lets the
            // Adam update overlap with the loss D2H copy (PyTorch-style ordering).
            optimizer.step();

            // SYNC POINT: now drain for loss value. Adam kernels queued above are
            // running concurrently with this sync, not after it.
            {
            ...
            std::cout << "step " << std::setw(5) << step 
                    ...
                    //   << " | optim_time: "  << (time_optim    * 1000.0) << "ms\n"
                      << std::endl;
