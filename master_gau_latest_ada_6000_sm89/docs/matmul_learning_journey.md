...
**What this is:** my from-scratch learning record on matrix multiplication — what
BLAS actually is, why the three levels have O(n), O(n²), O(n³) complexity, the
naive→optimized version ladder, the six loop orderings and which is best, the
"C must start at zero" rule, and the real CPU/GPU micro-benchmarks I ran to test
my own doubts. Written in plain English so future-me can re-derive all of it.
...
### Experiment B — does `ikj` (contiguous access, but forced memory RMW) beat `ijk` (register)?
> Faithful test: same launch config for both on each device.

```
CPU (N=512):  ijk(reg) 15.6 ms   vs  ikj(rmw) 7.4 ms    →  ikj wins 2.11×
GPU (N=2048): ijk(reg) 67.6 ms   vs  ikj(rmw) 60.3 ms   →  ikj wins 1.12× (≈ tie, ikj edges it)
```
- **CPU — `ikj` clearly wins (2.11×).** `ijk`'s register-C can't save it from **column-strided B
  cache misses**; `ikj`'s contiguous B & C access fixes that, and the CPU cache makes its
  C-read-modify-write **cheap** (contiguous, cache-resident). On CPU, **access pattern dominates.**
- **GPU — essentially a tie (`ikj` 1.12×).** The global-RMW penalty mostly **disappears** because
  each thread's `C[i][j]` is the **same address for every k** (loop-invariant) → it stays hot in
  **L1/L2** (and may be register-promoted) → it rarely actually travels to VRAM. So contiguous-`ikj`
  ties register-`ijk`.
- **Honest correction:** my earlier "register wins 2.85× on GPU" (Experiment A) was **not an
  apples-to-apples** comparison — it used a different block shape that gave the register kernel
  better B-reuse. With the **same launch config** for both (Experiment B), they're ~equal.

### The combined lesson (CPU vs GPU want different things)
| | what makes contiguous access cheap | best simple strategy |
|---|---|---|
| **CPU** | automatic L1/L2 cache + prefetcher | **`ikj`** — let the cache absorb the C RMW |
| **GPU** | nothing automatic — you build it | **register-accumulate + coalescing**; and tiling for the real win |

- The **global-RMW penalty only bites hard when the RMW target *changes*** (cold addresses → real
  VRAM trips). When the target is **reused / loop-invariant**, L1/L2 hides it.
- **Neither naive kernel is good:** both GPU versions are ~60 ms; a shared-memory **tiled** GEMM
  would be a few ms, and cuBLAS faster still. The naive ijk/ikj question is about *building correct
  intuition*; the real speed comes from **tiling + registers + coalescing together** (Part 3, V3–V7).
