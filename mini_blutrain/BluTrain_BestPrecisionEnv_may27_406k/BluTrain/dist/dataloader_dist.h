...
public:
    DataLoaderLite(int B, int T,
                     ...
                     int device_idx = 0)
        : B_(B), T_(T),
          dp_rank_(dp_rank), dp_world_(dp_world),
        ...
        if (dp_world_ <= 0 || dp_rank_ < 0 || dp_rank_ >= dp_world_)
            throw std::runtime_error("invalid dp_rank/dp_world");
    ...
    void reset() {
        ...
        // current_position = B*T*dp_rank
        pos_ = static_cast<size_t>(B_) * static_cast<size_t>(T_) * static_cast<size_t>(dp_rank_);
    }
    ...
    Batch next_batch() {
        ...
        if (pinned_x_) {
            ...
            cudaMemcpyAsync(input_tensor_.data(), pinned_x_, BT * sizeof(u_int16_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(target_tensor_.data(), pinned_y_, BT * sizeof(u_int16_t), cudaMemcpyHostToDevice, stream);
        } else {
        ...
        //current_position += B*T*dp_world
        pos_ += BT * static_cast<size_t>(dp_world_);

        // if current_position + (B*T*dp_world + 1) > len(tokens):
        //     load next shard; self.current_position = B*T*dp_rank
        if (pos_ + (BT * static_cast<size_t>(dp_world_) + 1) > shard_.size_tokens()) {
            advance_shard();
        }
    ...
    void skip_batches(size_t n) {
        const size_t BT = static_cast<size_t>(B_) * static_cast<size_t>(T_);
        const size_t batch_stride = BT * static_cast<size_t>(dp_world_);
...
private:
    void advance_shard() {
        ...
        const size_t BT = static_cast<size_t>(B_) * static_cast<size_t>(T_);
        pos_ = BT * static_cast<size_t>(dp_rank_);
    }

    int B_, T_;
    int dp_rank_, dp_world_;
